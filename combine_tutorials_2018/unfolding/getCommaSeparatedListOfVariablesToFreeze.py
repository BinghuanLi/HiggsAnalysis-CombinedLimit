...
def main():
    ...
    parser.add_argument( '--indexVals', action='store_true', help='return index vals')
    ...
    while cat:
        if cat.GetName().startswith("pdfindex"):
            ...
            indexVals.append(cat.GetName()+"="+str(cat.getIndex()))
        cat = catitr.Next()
    ...
    if DEBUG:
        ...
        if (args.indexVals): print ','.join(indexVals)
        else: print ','.join(varsToFreeze)
